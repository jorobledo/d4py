...
Modified:18/01/2023
    ...
    def __init__(self,params):
        ...
        self.beam = d4.setBeamInfo(zeroAngle=params.zeroAngle,wavelength=params.wavelength,
               LohenSlit=params.LohenSlit,GamsSlit=params.GamsSlit,topFlag=params.topFlag,
                              bottomFlag=params.bottomFlag)
        #--------1---------2---------3---------4---------5---------6---------7-----
        #help(d4.setBeamInfo)
        #--------1---------2---------3---------4---------5---------6---------7-----
        # Basic information about the container
        self.can = d4.setCanInfo(material=params.material,shape=params.shape,
              outerDiam=params.outerDiam,innerDiam=params.innerDiam,height=params.height)
        #--------1---------2---------3---------4---------5---------6---------7-----
        # Definition of the binning
        self.binning = d4.setBinInfo(AngularResolution=params.AngularResolution,
                       AMin=params.AMin, AMax=params.AMax, AStep=params.AStep,
                       QMin=params.QMin, QMax=params.QMax,  QStep=params.QStep,
        ...
        self.vana = d4.setVanaInfo(IncXS=auxVana['sig_inc'],CohXS=auxVana['sig_coh'],
                                    ScaXS=auxVana['sig_sca'],AbsXS=auxVana['sig_abs'],
                                    CohSL=auxVana['re_bcoh'],molarM=auxVana['weight'],NAtoms=1,
                                    Diam=params.Diam,Height=self.beam['height'],density=params.density)
        ...
        self.environmentData_qbin = d4.rebin(self.binning['Ares'],self.beam['wlength'],self.environmentData,*self.binning['Qbin'])
        self.containerData_qbin = d4.rebin(self.binning['Ares'],self.beam['wlength'],self.containerData,*self.binning['Qbin'])
        self.vanadiumData_qbin = d4.rebin(self.binning['Ares'],self.beam['wlength'],self.vanadiumData,*self.binning['Qbin'])
    ...
    def plotRaw(self):
        ...
        plt.subplot(2, 1, 1)
        plt.plot(self.environmentData[:,0],self.environmentData[:,1], 'r-', label='Environment')
        plt.plot(self.containerData[:,0],self.containerData[:,1], 'b-', label='Container')
        plt.plot(self.environmentData_qbin[:,0],self.environmentData_qbin[:,1], 'g-', label='Environment(Rebinned)')
        plt.plot(self.containerData_qbin[:,0],self.containerData_qbin[:,1], 'y-', label='Container(Rebinned)')
        ...
        plt.subplot(2, 1, 2)
        plt.plot(self.vanadiumData[:,0],self.vanadiumData[:,1], 'm-', label='Vanadium')
        plt.plot(self.vanadiumData_qbin[:,0],self.vanadiumData_qbin[:,1], 'y-', label='Vanadium(Rebinned)')
        plt.ylabel('$g(r)$')
        plt.legend(loc='best')
        plt.axis([None, None, None, None])
        plt.grid(True)
    ...
    def setting_bs_mc_d4vana(self,params):
        ...
        self.van_qbin_sub = d4.wsum2(1.0,self.vanadiumData_qbin,-1.0,self.environmentData_qbin)
        ...
        x_can, y_can = self.containerData_qbin[:,0], np.ones(len(self.environmentData_qbin[:,1]))
...
3.6499664E-02  5.6225456E-04 -1.8670474E-05"""
        np.savetxt(folder_generated+'/'+'Mcd8dq.dat', np.array((new_x, new_y, new_y_c)).T,
                   fmt='%.7e',header= header, delimiter='  ')
        ...
        for key, value in can_atoms_sample.items():
            can_elements[key]['MassNbr'] = d4.getMassNumber(float(can_elements[key]['weight']))
        ...
        mac_xs_sample = self.vana['den_aac'] * xs_can # va effden_acc? atomos/A-3 * barns / atomos
        ...
        XS_model_sample = [d4.XS_model(Eval=energia,
                                Eeff=Eeff,
                                composition_vec = cvals,
                                bound_xs_vec = sca_XS,
                                A_vec = avals) for energia in energias]

        XS_model_can = [d4.XS_model(Eval=energia, 
                             Eeff=Eeff, 
                             composition_vec = can_cvals,
                             bound_xs_vec = can_sca_XS,
                             A_vec = can_avals) for energia in energias]
        ...
        for i in range(0,max_count):
            ...
            angulo, single, s_aten, can, total = mc.mc2022('generated_files/salida',ncic=ncic,nt=nt,e0=E0,utab=U0,
                                            setot=xs_sample,sem=mac_xs_sample,pscat=p_scat_sample,
                                            setotc=xs_can,semc=mac_xs_van,pscatc=p_scat_can,
                                            r1 = r1/10,r2 = r2/10,
                                            h=h/10,dh=dh,ah=ah)
            qvals = d4.ang2q(angulo,wlength=float(params.wavelength)) 
            global_factor = generate_correction_factor(new_x, qvals, single[:,0], s_aten, total)
        ...
        heading = ['Background subtracted vanadium data',
                   'result =  vanadium - MTbellar',
                   '  Q(1/A)       Intensity            Error']
        file = 'vana_qbin_sub.dat'
        d4.saveFile_3col(file,self.van_qbin_sub_mc,heading)
        print(70*"-")         
        self.plot_bs_mc_results(energias,XS_model_can,xs_can,E0,qvals,single,s_aten,can,total)
        return
        
    def setting_model_d4vana(self,a0Val=True,a1Val=False,a2Val=True,a3Val=False,a4Val=True,
                                a0 = 1,a1 = 0.0,a2 = 0.001,a3=0.0, a4 =-0.0001,                
                                x_fit_min=1,x_fit_max=24.5,y_fit_min=9780,y_fit_max=14000):
        #----------Variable Area------------
        #-----------------------------------
...
#         y_fit_max=14000
        ...
        fitdata = d4.fit_range(x_fit_min,x_fit_max,y_fit_min,y_fit_max,self.van_qbin_sub_mc)
        #fitdata = d4.fit_range(0.36,23,15000,18000,van_qbin_sub)
        gmodel = lm.Model(d4.polyQ4)
        print('parameter names: {}'.format(gmodel.param_names))
        print('independent variables: {}'.format(gmodel.independent_vars))
        paramsGmodel = gmodel.make_params(a0=a0,a1=a1,a2=a2,a3=a3,a4=a4)
        ...
        result = gmodel.fit(fitdata[:,1],x=fitdata[:,0],a0=a0,a1=a1,a2=a2,a3=a3,a4=a4)
        ...
        vana_fit = (result.best_values['a0'],result.best_values['a1'],result.best_values['a2'],
                   result.best_values['a3'],result.best_values['a4'])
        ...
        print ('----> Model at Q={:4.2f} 1/A = {:10.3f}'.format(self.van_qbin_sub_mc[0,0],vanaInc[0]))
        print ('----> Factor for resolution = {:10.3f}'.format(vanaInc[0]/result.best_values['a0']))
        ...
        self.van_qbin_sub_nor[:,1] = self.van_qbin_sub_mc[:,1] * self.vana['SelfQ0'] /vana_n
        self.van_qbin_sub_nor[:,2] = self.van_qbin_sub_mc[:,2] * self.vana['SelfQ0'] /vana_n

        print('Low-Q limit for vanadium= {:.6f} barns/sterad/atom'.format(self.vana['SelfQ0']))
        print('High-Q limit for vanadium= {:.6f} barns/sterad/atom'.format(self.vana['FreeIncXS']/4/np.pi))
        
        self.plot_fit_results(vanaInc,fitdata,x,y,vana_r) 
        
        heading = ['Renormalisation of the vanadium data',
           'result =  vanadium',
           '  Q(1/A)       Intensity            Error']
    ...
    def plot_fit_results(self,vanaInc,fitdata,x,y,vana_inel):
        ...
        plt.plot(self.van_qbin_sub_mc[i:,0],self.van_qbin_sub_mc[i:,1], 'g-', label='(V+E)*Fmc')
        ...
        plt.figure(figsize=(8,5))
        plt.plot(self.van_qbin_sub_mc[i:,0],self.van_qbin_sub_mc[i:,1], 'bo', label='experimental data')
        ...
        plt.figure(figsize=(8,5))
        plt.errorbar(self.van_qbin_sub_nor[i:,0],self.van_qbin_sub_nor[i:,1], yerr=None, label='Vanadium')
        plt.plot(self.van_qbin_sub_nor[:,0],vanaInc* self.vana['SelfQ0']/vana_inel[1], 'r-', label='Model')
        plt.legend(loc='best')
        plt.title('Vanadium: Normalised data')
        plt.xlabel('$Q$ (${\AA}^{-1}$)')
        plt.ylabel('$d\sigma/d\Omega$ (barns/sterad/atom)')
        plt.grid(True)
        plt.tight_layout()
        plt.show()
    ...
    def plot_bs_mc_results(self,energias,XS_model_can,xs_can,E0,qvals,single,s_aten,can,total):
        

        
        plt.figure(figsize=(6,4))
        plt.plot(energias, XS_model_can, label='Vanadium model')
        plt.vlines(E0,3,7,linestyles='--', colors='red',alpha=0.5, label='D4 $E_0$ energy')
        ...
        plt.plot(self.vanadiumData_qbin[:,0],self.vanadiumData_qbin[:,1]*0.7, label='Vanadium')
        plt.plot(self.environmentData_qbin[:,0], self.environmentData_qbin[:,1], label='Environment')
        plt.plot(self.environmentData_qbin[:,0],(self.vanadiumData_qbin[:,1]- self.environmentData_qbin[:,1])*0.7, label='V-E')
        plt.plot(self.van_qbin_sub_mc[:,0], self.van_qbin_sub_mc[:,1], label='Vanadium corrected with ATT and MS')
        plt.xlabel('Q ($1/\AA$)')
        plt.ylabel('Intensity (arb. units)')
