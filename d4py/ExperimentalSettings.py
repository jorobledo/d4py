...
from .MiscelaneousCalculations import getCylVolume
...
# --------1---------2---------3---------4---------5---------6---------7---------
def setCanInfo(
    material="Vanadium", shape="Cylinder", outerDiam=5, innerDiam=4.8, height=60.0
):
...
def setBinInfo(
    ...
    binning["NbrPointsA"] = int(
        (binning["Abin"][1] - binning["Abin"][0]) / binning["Abin"][2]
    )
    binning["NbrPointsQ"] = int(
        (binning["Qbin"][1] - binning["Qbin"][0]) / binning["Qbin"][2]
    )
    binning["NbrPointsR"] = int(
        (binning["Rbin"][1] - binning["Rbin"][0]) / binning["Rbin"][2]
    )
    print(30 * "-")
    print("Binning")
...
def setNumorInfo(
    totalNumors=("Exp", 0, 1),
    containerNumors=("Can", 0, 1),
    environmentNumors=("Env", 0, 1),
    nickelNumors=("Ni5", 0, 1),
    vanadiumNumors=("Van", 0, 1),
    absorberNumors=("Abs", 0, 1),
...
def setVanaInfo(
    ...
    vana["SelfQ0"] = vana["IncXS"] / 4.0 / np.pi
    ...
    vana["den_aac"] = getAtomicDensity(
        density=vana["den_gcc"], molarMass=vana["molarM"]
    )
...
def setSampleInfo(
    ...
    sample["volumeInBeam"] = (
        getCylVolume(diameter=sample["diam"], height=sample["heightInBeam"]) / 1000.0
    )
    ...
    sample["den_aac"] = getAtomicDensity(
        density=sample["den_gcc"], molarMass=sample["molarM"]
    )

    # Effective atomic density (atoms/A3)
    sample["effden_aac"] = getAtomicDensity(
        density=sample["effden_gcc"], molarMass=sample["molarM"]
    )
